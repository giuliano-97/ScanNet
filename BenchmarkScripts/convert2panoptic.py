...
def convert2panoptic(scannetPath, outputFolder=None):
    ...
    if not files:
        print(
            "Did not find any files for using matching pattern {}. Please consult the README.".format(
                search
            )
        )
        sys.exit(-1)
    ...
    print(
        "Json file with the annotations in panoptic format will be saved in {}".format(
            outFile
        )
    )
    ...
    if not os.path.isdir(panopticFolder):
        print(
            "Creating folder {} for panoptic segmentation PNGs".format(panopticFolder)
        )
        os.mkdir(panopticFolder)
    print(
        "Corresponding segmentations in .png format will be saved in {}".format(
            panopticFolder
        )
    )
    ...
    for progress, f in enumerate(files):
        ...
        print(
            "\rProgress: {:>3.2f} %".format((progress + 1) * 100 / len(files)), end=" "
        )
        sys.stdout.flush()
