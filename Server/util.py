...
def call(cmd, log, rundir="", env=None, desc=None, testMode=None):
    ...
    try:
        ...
        log.info(
            "Time=" + str(datetime.timedelta(seconds=delta_time)) + " for " + desc_str
        )
        res = prog.returncode
...
def list_files(dirname):
    ...
    for file in files:
        ...
        mtimestr = datetime.datetime.fromtimestamp(st.st_mtime).strftime(
            "%Y-%m-%d %H:%M:%S.%f"
        )[:-3]
...
def lastModified(fileinfos):
    ...
    for fileinfo in fileinfos:
        if last == None or fileinfo.get("modifiedAtMillis") > last.get(
            "modifiedAtMillis"
        ):
            last = fileinfo
    return last
...
def millisToIso(millis):
    ...
    mtimestr = datetime.datetime.fromtimestamp(millis / 1000.0).strftime(
        "%Y-%m-%dT%H:%M:%S"
    )
    return mtimestr
...
def make_tree(base_dir, path):
    tree = dict(
        ...
        relative_name=path.replace(base_dir, "received"),
        children=[],
    )
    try:
        lst = os.listdir(path)
        ...
        for name in lst:
            ...
            if os.path.isdir(fn):
                ...
                tree["children"].append(
                    dict(
                        name=name,
                        fileSize=naturalsize(st.st_size),
                        modifiedAt=mtimestr,
                        relative_name=os.path.join(
                            path.replace(base_dir, "received"), name
                        ),
                    )
                )
    return tree
