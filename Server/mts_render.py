...
def render(args):
    ...
    sensor = pmgr.create(
        {
            ...
            "sampler": {"type": "ldsampler", "sampleCount": args.get("samples")},
            "fov": 45.0  # ,
    ...
    if args.get("render_turntable"):  # render turntable sequence
        ...
        for i in range(0, 360 / angleDelta, stepSize):
            rotationCur = (
                camTranslate * Transform.rotate(UP, i * angleDelta) * camUnTranslate
            )
            trafoCur = Transform.lookAt(
                rotationCur * camOrigin, camTarget, rotationCur * camUp
            )
        ...
        # convert to video
        os.system(
            "ffmpeg -y -pix_fmt yuv420p -r 30 -i {0}_%03d.png -vcodec libx264 {0}.mp4".format(
                meshfile
            )
        )
    else:  # render single view
        rotationCurr = Transform.rotate(UP, args.get("theta"))
        transformCurr = Transform.lookAt(
            rotationCurr * camOrigin, camTarget, rotationCurr * camUp
        )
...
def main():
    ...
    parser.add_argument(
        "--cameras", dest="cameras", action="store", help="Cameras file to render"
    )
    parser.add_argument(
        "--integrator",
        dest="integrator",
