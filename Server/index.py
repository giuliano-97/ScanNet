...
def check_stages(stages_data, meta, times=None):
    ...
    if meta.get("files"):
        ...
        for stage in stages:
            ...
            if stage.get("output"):
                outputOk = check_files(
                    filesByName, stage.get("output"), stage.get("outputCheck") == "any"
                )
                if outputOk:
                    ok = True
            ...
            if ok is None:
                ...
                if inputOk:
                    ...
                    outdated = any(
                        f.get("modifiedAtMillis") < lastModifiedMillis for f in outputs
                    )
...
def indexAndSave(args, loadFn, saveFn):
    ...
    if args.get("single"):
        ...
        index_single(
            args.get("input"), subdir, lambda r: assignItem(rows, r["id"], r), args
        )
    elif args.get("recursive"):
        index_all_recursive(
            args.get("input"), lambda r: assignItem(rows, r["id"], r), args
        )
    else:
...
def main():
    ...
    parser.add_argument(
        "-o", "--output", dest="output", action="store", help="Output CSV filename"
    )
    parser.add_argument(
        "-s",
        "--single",
