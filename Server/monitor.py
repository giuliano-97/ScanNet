...
def index_scan(dirname):
    ...
    with INDEX_LOCK:
        ...
        indexed = index.index(
            {
                "input": path,
                "output": indexfile,
                "root": app.config["STAGING_FOLDER"],
                "single": True,
                "append": True,
                "checkCleaned": True,
        ...
        if indexed:
            res = post(
                WEBUI + "/scans/populate?group=staging&replace=true",
                indexed.values(),
                log,
            )
...
def convert_video(dirname):
    ...
    with CONVERT_VIDEO_LOCK:
        ret1 = util.call(
            CMD_ARGS + [CONVERT_H264_TO_MP4_BIN, "--skip-done", path],
            log,
            desc="h264-to-mp4",
        )
        ret2 = util.call(
            CMD_ARGS + [CONVERT_H264_TO_THUMBNAIL_BIN, "--skip-done", path],
            log,
            desc="h264-thumbnail",
        )
        if ret1 < 0 or ret2 < 0:
...
def main():
    ...
    parser.add_argument(
        "--port", dest="port", action="store", default=5001, help="Port number"
    )
