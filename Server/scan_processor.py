...
RECONS_BIN = os.path.join(
    RECONS_DIR, "FriedLiver.exe zParametersScanNet.txt zParametersBundlingScanNet.txt"
)
...
VOXELHASHING_BIN = os.path.join(
    VOXELHASHING_DIR,
    "DepthSensing.exe zParametersScanNet.txt zParametersTrackingDefault.txt",
)
...
THUMBNAIL_BIN = (
    cfg.GIT_BASH + " " + os.path.join(cfg.SCRIPT_DIR, "scripts", "thumbnail.sh -v")
)
...
def process_scan_dir(path, name, config):
    ...
    if not (
        validNovh
        or os.path.isfile(namebase + ".depth")
        or os.path.isfile(name + ".sens")
    ):
...
def process_scan_dir_basic(path, name, config):
    ...
    if config.get("overwrite") or not os.path.isfile(sensfile):
        if config.get("convert"):
            ret = util.call(
                [CONVERTER_BIN, path, uncalibrated_sensfile],
                log,
                CONVERTER_DIR,
                desc="convert",
            )
            if not os.path.isfile(uncalibrated_sensfile) and not TEST_MODE:
                return (
                    "Scan at %s aborted: no uncalibrated sens file (convert failed)"
                    % path
                )
        ...
        if config.get("calibrate"):
            if not os.path.isfile(uncalibrated_sensfile) and not TEST_MODE:
                return (
                    "Scan at %s aborted: no uncalibrated sens file for calibrate" % path
                )
            ret = util.call(
                [
                    CALIBRATE_BIN,
                    uncalibrated_sensfile,
                    sensfile,
                    DEVICES_CSV,
                    DEVICES_DIR,
                ],
    ...
    if config.get("recons"):
        if not os.path.isfile(sensfile) and not TEST_MODE:
            return (
                "Scan at %s aborted: no calibrated sens file (calibrate failed)" % path
            )
    ...
    if config.get("improve"):
        ret = util.call(
            VOXELHASHING_BIN + " " + sensfile, log, VOXELHASHING_DIR, desc="improve"
        )
    ...
    if config.get("segment"):
        ret = util.call(
            SEGMENT_BIN + " " + decimated_mesh, log, SEGMENT_DIR, desc="segment"
        )

    # Generate images
    if config.get("render"):
        ret = util.call(
            RENDER_BIN + " " + decimated_mesh, log, RENDER_DIR, desc="render"
        )

    # Generate thumbnails
    if config.get("thumbnail"):
        ret = util.call(
            THUMBNAIL_BIN + " " + path.replace("\\", "/"),
            log,
            env=dict(
                os.environ,
                CONVERT=IMG_CONVERT_BIN,
                PNGQUANT=PNGQUANT_BIN,
                OPTIPNG=OPTIPNG_BIN,
            ),
            desc="thumbnail",
        )
