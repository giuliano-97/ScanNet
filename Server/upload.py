...
SCAN_PROCESS_TRIGGER_URL = (
    cfg.DATA_SERVER + "/scans/process/"
)  # WebUI process url is nonblocking (it places it on a queue)
...
def trigger_video_conversion(basename, log):
    ...
    try:
        ...
        log.warning(
            "Error converting video " + convert_video_url + ": " + str(e.reason)
        )
...
def trigger_processing(basename, log):
    ...
    log.info(
        "Calling scan process script for " + basename + " at " + process_url + " ..."
    )
    try:
...
def receive_file(request, filename, output=None):
    ...
    while True:
        if content_read % (chunk_size * 1000) == 0:
            log.info(
                "Receiving %s: Uploaded count: %d, \t Percent: %.2f",
                filename,
                content_read,
            ...
            if scan_done_uploading(stagingdir):
                ...
                if auto_process_scan:
                    ...
                    indexThread = threading.Thread(
                        target=preprocess, args=(basename, log)
                    )
                    indexThread.start()
                    processThread = threading.Thread(
                        target=trigger_processing, args=(basename, log)
                    )
                    processThread.start()
            return util.ret_ok()
        else:
...
def verify_file():
    ...
    if valid:
        ...
        log.error(
            "File %s: hash mismatch. Given: %s, calculated: %s",
            filename,
            checksum,
            calculated_checksum,
        )
        raise Error(
            message=(
                "File hash mismatch. Given: %s, calculated: %s"
